      ...
      <div class="grid grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center gap-3">
        ...
        <div class="text-center shrink-0 px-2">
          ...
          <div v-if="editable && !isLocked(p)" @click.stop.prevent>
            ...
            <div v-if="crowdEnabled" class="mt-1"><CrowdLine :match-id="p.matchId" :totals="crowdTotals" :league-totals="leagueTotals" :league-active="leagueActive" /></div>
            <MatchOdds v-if="oddsEnabled" :odds="oddsByMatch[p.matchId] ?? null" class="justify-center mt-1" />
          </div>
          <template v-else>
