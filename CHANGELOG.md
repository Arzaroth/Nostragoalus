...
- The Players tab of a rugby match lists everyone who scored, not only try scorers. A fly-half who kicked all of his side's points was missing from it entirely. Each player shows tries and points, and the side's leading points scorer takes the place of a top assister - rugby does not record an assist.
- A rugby scorer who was not in the tournament squad list appeared as a blank line with a score beside it. Those players are now looked up one by one, and the matches already stored are read again so their names appear.
...
- Admin: rugby tournaments can be added, from World Rugby's own match feed rather than a third party. Pools, the knockout rounds and the bronze final all come through correctly, and the 2027 Rugby World Cup - including its new round of 16 - already reads in full.
...
- The third-place play-off no longer shows "Final counts double" and no longer doubles points: a match's stage is now frozen when first synced, so a provider relabelling the tie can't drift it into being treated as the final.
...
### Added
- **The Equalizer bot**: a new bot that calls a draw in every single match. Enable it (and the Crowd Bot) from a single "Bots" button on the leaderboard - each shows up as a ghost row at the rank it would have earned, with its own page listing every pick. It never affects anyone's real standing.
- **Evil Twin on player profiles**: every player's profile has an "Evil Twin" toggle - their own picks with every score flipped (a draw is its own twin) - so you can see how betting against them would have gone, and where that twin would rank. Follows the same visibility as their picks.
...
- **Tied players share a rank**: in the Stats boards, players level on goals (or assists) now show the same position number, with the next player skipping ahead (1, 2, 2, 2, 2, 6...), instead of being numbered off arbitrarily.
...
- **Reply to messages**: reply to any message and the original is quoted above your answer (and above the composer while you write it), so a thread is easy to follow. The quote is clickable - it jumps to and highlights the original post. The quote is decrypted on your device like everything else.
- **Images in chat**: drag and drop, paste, or attach an image (up to 5 MB). It is downscaled to webp and end-to-end encrypted on your device before sending, so the site only ever stores scrambled bytes - it never sees the picture. Tap an image to enlarge it, then copy, download, or share it.
- **Edit and delete your own messages**: fix a message after sending - it shows an "edited" marker with the time, and the new text is re-encrypted on your device and updates live for everyone - or delete it, which shows as "message removed" and keeps any replies intact. You can also withdraw a report you made.
- **Report and moderate chat messages**: anyone can report a message they did not send. Once enough distinct members flag one (a quarter of the league, at least three) it auto-hides as "pending review" for everyone except the owner and moderators. Owners and moderators get a Reports list to read each flagged message and either keep it or remove it, and can remove any message directly. Removed messages show as "message removed" and keep replies intact. Reports and removals reflect live for everyone.
...
### Added
- **Tamper-evident scores**: every prediction change is now sealed into an append-only, hash-chained ledger (commit-reveal). Picks stay hidden until kickoff, then their score and salt are revealed so anyone can re-open the seal. The new public **Verify scores** page (footer link) pulls the whole ledger and recomputes the chain in your own browser - tamper with any sealed entry and the chain stops reproducing the head you snapshotted today. Predictors appear as an opaque hash, never their account. Your browser also remembers the last chain state it checked (kept on your device, never sent to us), so a later edit to anything you have already seen is caught automatically and a warning surfaces site-wide - no need to save the hash yourself.
