...
describe('normalizeUefaTimeline', () => {
  ...
  it('drops noise and unknown-side actors; second yellows; sub without a replacement', () => {
    ...
    const evs: UefaEvent[] = [
      ...
      { type: 'CORNER', time: { minute: 44 }, timestamp: at(11), primaryActor: actor('2', 'p8', 'W') }, // corner: taker, away side
      ...
      { type: 'CORNER', timestamp: at(22), primaryActor: { team: { id: null } } }, // no side, dropped
    ]
    const tl = normalizeUefaTimeline(evs, '1', '2')
    expect(tl.map((e) => e.kind)).toEqual(['sub', 'corner', 'foul', 'shot', 'shot', 'shot', 'red', 'second-yellow', 'second-yellow', 'yellow'])
    expect(tl.find((e) => e.kind === 'corner')).toMatchObject({ side: 'AWAY', playerName: 'W', minute: "44'" })
