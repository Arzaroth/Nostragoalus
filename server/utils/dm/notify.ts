...
// there is no preview. dedupeKey makes re-delivery idempotent. Best-effort: the
// caller fires and forgets so a delivery hiccup never fails the message post.
export async function notifyDm(
  db: AppDatabase,
  opts: { threadId: string; messageId: string; senderId: string; recipientId: string },
): Promise<boolean> {
