...
describe('migrateBlobsToStorage', () => {
  it('moves legacy chat ciphertext into storage and clears the column', async () => {
    ...
    expect(await migrateBlobsToStorage(db, storage)).toEqual({ chatMigrated: 1, avatarMigrated: 0 })
  ...
  it('moves data: avatars into storage and rewrites user.image to the serving url', async () => {
    ...
    const storage = memoryStorage()
    expect(await migrateBlobsToStorage(db, storage)).toEqual({ chatMigrated: 0, avatarMigrated: 1 })
  ...
  it('is idempotent: already-migrated rows and external avatars are left alone', async () => {
    ...
    expect(await migrateBlobsToStorage(db, storage)).toEqual({ chatMigrated: 0, avatarMigrated: 0 })
  ...
  it('migrates across multiple batches', async () => {
    ...
    const storage = memoryStorage()
    expect(await migrateBlobsToStorage(db, storage, { batchSize: 2 })).toEqual({ chatMigrated: 5, avatarMigrated: 0 })
