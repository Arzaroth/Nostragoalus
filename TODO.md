...
- [ ] **Fergie time drops any match that went to extra time**: `computeFergie`
      reconciles the goal_event tally against `fullTimeHome/Away`, which stores the
      REGULATION score only (`server/utils/sync/upsert-matches.ts`), while goal_event
      carries ET goals too. So a knockout tie with a genuine 90'+ stoppage swing that
      then goes to ET fails reconcile (tally > regulation score) and contributes zero
      Fergie time. Safe-fail (never a wrong number, just a conservative miss). To
      recover it, replay against the regulation line: stop the running score at the
      end of 90' (+stoppage) and reconcile there, and keep `isAddedTime` scoped to
      base >= 90 (already done - it currently also admits ET stoppage like `105'+2'`,
      harmless only because those matches drop on reconcile today).
- [ ] **Fergie time anchors on the current scoring config, not the scored version**:
      `loadFergieTime` re-derives points with `getScoringConfigFor` (current active
      version) while `prediction.totalPoints` was persisted at `scoredAtVersion`. If an
      admin edits the scoring override, or a late ODDS BACKFILL snapshot lands, between
      a match being scored and the next finalize tick rescoring it, the Fergie net is
      computed against different rules than the leaderboard shows. Transient
      (self-heals on the next finalize tick); low priority.

Resolved by the Fergie time v2 rework: the ad-hoc `minute?.includes('+')` now reuses
the canonical `minuteValue()` parser, and `fergieSwing()` was removed (the metric is
per-match `FergieMatch`, not a `highlight()`-shaped swing).
...
DMs reuse `chat_message` (scoped by `dm_thread_id`) and now share ChatPanel + the
chat services, so they have full parity with league chat (reactions, reply,
threads, edit, images, media gallery, link previews) inside a Direct mode of the
one messaging dock. Still open:

- [x] **DM-scoped message features wired** (done): reactions, attachments/images,
      link previews, threads and edit now work on DM messages, reusing ChatPanel +
      the generalized chat services (scope-agnostic via `server/utils/chat/access.ts`).
- [x] **One messaging surface** (done): DMs were consolidated into `ChatDock` as a
      Chat | Direct mode reusing ChatPanel, superseding the standalone `DmDock`
      (deleted). The dock is now available to any signed-in user, not league-gated.
- [ ] **@mentions in DMs**: intentionally omitted (a 1:1 room has no roster to
      mention). Revisit only if group DMs ever land.
...
- [x] **`onboarding.e2e.ts` "the onboarding tour auto-starts" was red on `master`**
      (fixed on `fix/onboarding-autostart`): it was a test bug, not an app bug. The
      tour auto-starts off an in-session signal fired only when the league prompt is
      dismissed in the current page-load; the spec let `signUp` dismiss the prompt on
      one page, then navigated to `/matches` (wiping the module-level signal) and
      expected the tour there. Fixed by leaving the prompt pending through `signUp`
      (`dismissPrompt: false`) and dismissing it on the matches page itself. The
      2.20.0 auto-start gating was correct and is unchanged. (Surfaced once the
      docker-project-name dot fix let the full suite run from a worktree.)
