...
- Admin : lister les compétitions d'un fournisseur est nettement plus rapide, et une limite de débit s'applique désormais réellement quand plusieurs requêtes sont en vol, et non à une seule. ESPN nomme une ligue par requête et en diffuse plus de deux cents, soit vingt secondes derrière un bouton sans rien à afficher : cela passait pour une panne.
...
- Un membre de ligue sans nom d'affichage corrompait toutes les mentions @ ecrites dans la discussion de cette ligue sur le site.
...
- Application Android : si le serveur ne prend plus en charge la version installée, l'application le dit clairement et renvoie vers la version actuelle, au lieu d'échouer écran par écran avec des erreurs incompréhensibles.
...
- Le panneau de téléchargement Android de la page « À propos » aligne sa version, sa taille et sa date de publication en colonne au lieu de les décaler.
