# Testing and the merge gate

Nothing merges without a green gate. This file is what the gate runs, how
coverage is scoped, and the test harness for services and components.

## The gate

`mise run check` (a web-app task, `apps/web-nuxt/.mise.toml`) runs all four
steps; run them individually when iterating (beware zsh pipelines masking exit
codes - check steps separately or `set -o pipefail`).
`.github/workflows/ci.yml` and `mise run release` both run the same four steps
in order. The `pnpm` scripts run from `apps/web-nuxt` (or
`pnpm -C apps/web-nuxt <script>`):

1. `pnpm typecheck` - Nuxt/vue-tsc type check.
2. `pnpm test:coverage` - `vitest run --project unit --coverage`, with the
   **98%** threshold enforced (lines, functions, statements, branches).
3. `pnpm test:components` - `vitest run --project nuxt` (components and
   composables).
4. `pnpm build` - the prod SSR/rollup build. It catches unresolved-import link
   errors the other steps miss (import shared via the `#shared` alias, not deep
   `../../../../shared/*`, from nested route pages).

`pnpm typecheck` is two checks behind one name: `nuxt typecheck` for app/ and
server/, then `tsc -p tsconfig.canary.json` for `scripts/canary/` + its tests,
which neither of Nuxt's generated tsconfigs reaches.

The gate is **offline on purpose**, and so is `.github/workflows/ci.yml`. The one
thing it therefore cannot see is an upstream feed changing shape under the
providers; that is the daily canary's job, off this gate entirely -
[provider-canary.md](provider-canary.md).

## Coverage scope (and why it shapes the code)

`apps/web-nuxt/vitest.config.ts` defines two projects:

- **unit** (`name: 'unit'`, node env) - includes
  `{server,lib,db,shared,tests,app}/**/*.test.ts`, excludes `*.nuxt.test.ts`,
  30 s test/hook timeouts, and spells out the `#shared` alias (the plain-node
  project has no Nuxt resolver). This project carries the coverage gate, and
  also runs the OpenAPI contract test (`tests/contract/`), the golden-vector
  parity tests (`tests/parity/`, see
  [cross-stack-contract.md](cross-stack-contract.md)) and the canary tests
  (`tests/canary/`).
- **nuxt** (`name: 'nuxt'`, Nuxt env) - includes `apps/web-nuxt/app/**/*.nuxt.test.ts`. No
  coverage is collected here. Its setup file `apps/web-nuxt/tests/setup-nuxt.ts`
  drains pending macrotasks after each test, so PrimeVue's deferred tooltip
  removal cannot fire after JSDOM teardown and fail the run with every test
  green (a load-dependent flake that once failed a release).

Coverage **includes** the three logic surfaces: `apps/web-nuxt/server/utils/**`, `apps/web-nuxt/shared/**`,
`apps/web-nuxt/app/utils/**`. It **excludes**: `*.test.ts`, `**/types/**`, the runtimeConfig
glue files `apps/web-nuxt/server/utils/providers/index.ts` and `apps/web-nuxt/server/utils/storage/index.ts`,
`apps/web-nuxt/server/utils/app-download/index.ts`,
`apps/web-nuxt/server/utils/http.ts`, `apps/web-nuxt/server/utils/share/og-assets.ts` (storage + Google Fonts
fetch glue, covered via the OG routes), `apps/web-nuxt/app/utils/image.ts` (canvas/Image DOM
glue with no headless surface) and `apps/web-nuxt/app/utils/download.ts` (anchor-click
DOM glue, exercised by the e2e downloads).

The gate covers logic dirs only, NOT `apps/web-nuxt/server/api` routes, `apps/web-nuxt/server/tasks`, or
`apps/web-nuxt/app/pages`. That is the whole reason routes and pages stay thin and logic lives
in services (see [overview.md](overview.md) and [server.md](server.md)).

## Service tests (pglite)

`apps/web-nuxt/tests/db.ts` `createTestDb()` spins up an in-memory Postgres via
`@electric-sql/pglite` with the **real** Drizzle migrations from `./drizzle`,
so service tests exercise the actual schema, constraints and enums (see
[database.md](database.md)). The migrations run once per worker process into a
snapshot (`dumpDataDir`) that every later test DB restores via `loadDataDir`
(~140 ms instead of ~1 s each). It returns `{ db, client }`.

`apps/web-nuxt/tests/factories.ts` provides builders: `makeUser`, `makeCompetition`,
`seedCompetition`, `makeMatch`, `makeGoalEvent`, `makePrediction`,
`makeLeaguePrediction`, `makeLeague`, `addLeagueMember`, `makeReaction`. `apps/web-nuxt/tests/storage.ts` provides `memoryStorage()` for services that
touch image blobs (see [storage.md](storage.md)).

## Component tests

`*.nuxt.test.ts` files use `mountSuspended` from `@nuxt/test-utils`. Two rules
that prevent order-dependent flakes:

- Unmount what you mount.
- Clear the vue-query cache between tests. Leaked observers plus the app-level
  60s `staleTime` (see [client.md](client.md)) have both caused flakes where a
  later test reads a previous test's cached data.

These specs stub the data layer with `vi.stubGlobal('$fetch', ...)`. That only
works because of a VITEST-only `app:templates` override in
`apps/web-nuxt/nuxt.config.ts`: since Nuxt 4.5 `$fetch` is an auto-import backed
by a generated `fetch.mjs` that snapshots `globalThis.$fetch` at module-eval
time, so an unpatched build binds the real ofetch before any stub runs. The
override rewrites that template to resolve the global per call. The alternative,
if the override ever fights an upgrade, is `mockNuxtImport('$fetch', ...)` per
spec.

## End-to-end (Playwright)

A browser e2e suite runs OUT-OF-BAND from the 98% merge gate: it needs a Docker
stack, so it is not part of the gate or CI. Run it with
`mise run e2e` (which brings the stack up via `e2e-up`; tear it down afterward
with `e2e-down`); `mise run e2e-bun` runs the same suite against the app under
the Bun runtime (`e2e-up-bun`, `compose.e2e-bun.yaml`) - one or the other, they
share ports. Specs are `apps/web-nuxt/tests/e2e/*.e2e.ts` (seed via
`helpers/db.ts`, sign in via `helpers/auth.ts`, plus `helpers/maildev.ts` and
`helpers/sso.ts`), configured by `apps/web-nuxt/playwright.config.ts`: serial
(`workers: 1`, the specs share one DB), 120 s per test, one retry on CI only.
The legacy `smtp-otp.e2e.mjs` node script is not collected; it runs via
`pnpm e2e:smtp`.
It exercises full flows the unit and component tests cannot: predict -> finalize
-> leaderboard, the password-reset and delete-account mail flows (against maildev),
SSO/OIDC login (against a dockerized Keycloak IdP, trusted via
`NUXT_SSO_TRUSTED_ORIGINS` - see [auth.md](auth.md)), and the meta-pick pickers
(`apps/web-nuxt/tests/e2e/pickers.e2e.ts` drives the champion and best-scorer pickers in a real
browser, asserting the shared `MetaPickShowcase` frame reacts to a live pick). It
runs against an isolated, disposable stack - the `ng-e2e-<checkout dir>` compose
project (dots mapped to dashes) with shifted ports and its OWN empty database -
so it never touches dev pgdata.

`apps/web-nuxt/tests/e2e/global-setup.ts` makes the suite reproducible from that empty DB: it
waits for the app, warms the cold HMR routes the first specs hit (a route compiles
on first request under the dev server), ensures the admin account, pins email
verification on via the admin API, and seeds the default `scoring_config` (also
seeded app-side on boot - see [server.md](server.md)). `apps/web-nuxt/nuxt.config.ts`
`vite.optimizeDeps.include` pre-bundles the client deps Vite would otherwise
discover mid-session and respond to with a full reload, which would drop an
in-flight sign-up POST or verify-email navigation and flake the browser run.

## Sources

- `apps/web-nuxt/vitest.config.ts`, `apps/web-nuxt/tests/setup-nuxt.ts`, `apps/web-nuxt/package.json` (scripts), `mise-tasks/release`, `.github/workflows/ci.yml`, `.mise.toml` (`e2e*` tasks)
- `apps/web-nuxt/tests/db.ts`, `apps/web-nuxt/tests/factories.ts`, `apps/web-nuxt/tests/storage.ts`
- `apps/web-nuxt/playwright.config.ts`, `apps/web-nuxt/tests/e2e/**` (incl. `apps/web-nuxt/tests/e2e/README.md`, `global-setup.ts`, `pickers.e2e.ts`), `apps/web-nuxt/.env.e2e`, `apps/web-nuxt/compose.e2e.yaml`, `apps/web-nuxt/compose.e2e-bun.yaml`
- `apps/web-nuxt/nuxt.config.ts` (`vite.optimizeDeps.include` dev pre-bundle)
