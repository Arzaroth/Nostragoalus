...
Chat and DMs are end-to-end encrypted **in the browser** (`apps/web-nuxt/app/utils/e2ee.ts`,
...
- Real Subresource Integrity (`integrity=` on the app's own chunks) is not
  ...
  reasoning as the deferred script-src CSP in `apps/web-nuxt/nuxt.config.ts`).
