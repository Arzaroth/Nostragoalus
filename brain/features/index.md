...
| [Rugby (multi-sport)](rugby.md) | `competition.sport` drives the provider, the scoring preset, the ranking table and the header mark; rugby union is the first non-football sport | unreleased | `competition.sport`/`provider_sport`, `scoring_config.margin_bands`, `goal_event.points`; `providers/worldrugby.ts`, `shared/sport.ts` |
...
| [Personal analytics](analytics.md) | Per-competition bias report on your own picks: tier mix, goals lean, home/draw lean, over/under-rated teams, accuracy sparkline, streak, best call + biggest miss; works mid-tournament | unreleased | `server/utils/analytics/service.ts`, `me/analytics.get.ts`, `AnalyticsReport.vue`, `useAnalytics.ts` |
| [Head-to-head](head-to-head.md) | Compare two players over their shared scored picks: points, W-L-T record, pick agreement, cumulative lead chart, biggest divergences; profile "Compare" entry | unreleased | `server/utils/analytics/h2h.ts`, `head-to-head.get.ts`, `H2HReport.vue`, `useHeadToHead.ts`, `compare.vue` |
...
| [Connected devices](connected-devices.md) | Account page lists active login sessions (device/IP/last-active) with per-device and bulk sign-out | unreleased | `session.ipAddress/userAgent`; `useSessions.ts`, `app/utils/user-agent.ts`, `account.vue` |
...
| [Rewards](rewards.md) | Per-league real-world prizes over 11 criteria (owner add/delete), per-league Team Specialist team, live winner standings, "prizes you hold", owner/moderator winners CSV export (the one route disclosing member emails) | v2.14.0 | `league_reward`; `server/utils/rewards/{service,criteria}.ts` |
