...
- [x] Dart `e2ee` port (parity-verified) - BOTH halves ported to `lib/e2ee/e2ee.dart`
  and GREEN on the emulator with native libsodium: decrypt against the frozen KATs,
  encrypt via a round-trip through the proven decrypt path.
- [~] League chat (send / receive / history) - full data layer + UI built (identity
  bootstrap, per-epoch group-key open, decrypt list, encrypt+send, request-key,
