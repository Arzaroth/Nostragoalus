// Dart port of the play-by-play labelling in apps/web-nuxt/app/utils/match-view.ts
// (`pbpTextSpec`, `TIMELINE_ICONS`, `isGoalKind`) plus `formatPlayerName` from
// app/utils/format.ts. Frozen by shared/parity-json/match-view.json; the Flutter
// app imports this copy, so what it renders is what the vectors prove.
...
class PbpSpec {
  ...
  /// The shape the frozen vectors carry (optional members absent when null).
...
PbpSpec pbpTextSpec({
  ...
  if (kind == 'period') {
    final period = periodKind == null || periodKind.isEmpty ? null : _periodKeys[periodKind];
    return PbpSpec(period == null ? '' : 'match.pbp.period.$period');
  }
  if (kind == 'var') {
  ...
  if (kind == 'sub') {
    final on = playerInName ?? '';
    final off = playerOutName ?? '';
    if (on.isNotEmpty && off.isNotEmpty) {
      return PbpSpec('match.pbp.sub', params: {
        'playerIn': formatPlayerName(on),
        'playerOut': formatPlayerName(off),
      });
    }
