...
const homeScore = computed(() => {
  ...
  return isLive.value ? liveHeaderScore(homeGoalEvents.value.length, stored, feedReady.value) : stored
})
const awayScore = computed(() => {
  const stored = live.value?.fullTimeAway ?? m.value?.fullTimeAway ?? null
  return isLive.value ? liveHeaderScore(awayGoalEvents.value.length, stored, feedReady.value) : stored
})
// Celebrate off the score the header actually shows (feed-led, above) so the pixel
// goal fires as the scoreline moves - and a VAR disallow that drops the score never
// fires it. The first non-null reading just seeds the baseline, so opening a match
// already in play doesn't set it off.
  ...
  <div v-if="m" class="flex flex-col gap-6">
    ...
    <div class="rounded-2xl border p-6" style="background: var(--p-content-background); border-color: var(--p-content-border-color)">
      ...
      <div v-if="eventsReady && timeline.length" class="grid grid-cols-[1fr_auto_auto_auto_1fr] gap-x-1.5 gap-y-1 mt-2 pt-3 border-t text-xs items-center" style="color: var(--p-text-muted-color); border-color: var(--p-content-border-color)">
        <template v-for="(e, i) in timeline" :key="i">
          ...
          <span class="w-4 flex justify-center">
            <template v-if="e.side === 'HOME'">
              <template v-if="e.kind === 'goal'">{{ scoreIcon(selectedSport, e.points) }}</template>
      ...
      <div v-if="canPredict || myPred" class="flex flex-col items-center gap-1.5 mt-4 pt-3 border-t text-sm" style="border-color: var(--p-content-border-color)">
        ...
        <!-- Owner-only: an earlier pick of yours would have scored better. Live
             while the match is in play, resolving to the full-time line at the end. -->
        <PastPickHint v-if="myPred && hasStarted" :match-id="id" :started="hasStarted" :live="isLive" :live-key="scoreTotal" />
      ...
      <!-- The match thread now lives in the floating ChatDock (Global/Match toggle). -->
    </div>
