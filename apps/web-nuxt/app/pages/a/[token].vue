...
const { data, error } = await useFetch<AnalyticsSummary>(() => `/api/share/analytics/${token.value}`)
...
// PNG so the LINK unfurls with a bias preview in chats.
const origin = useRequestURL().origin
const imageUrl = computed(() => `${origin}/og/analytics/${token.value}`)
const pageUrl = computed(() => `${origin}/a/${token.value}`)

const card = computed(() => data.value?.card ?? null)
const title = computed(() =>
  card.value ? t('share.analyticsLanding.title', { name: card.value.displayName }) : 'Nostragoalus',
)
const description = computed(() => t('share.analyticsLanding.subtitle'))
