            ...
            <template v-if="session && session.data">
              ...
              <Popover ref="userMenu" @show="onUserMenuShow" @hide="onUserMenuHide">
                <div class="flex flex-col w-52 -m-1">
                  ...
                  <NuxtLink v-if="slug" :to="`/${slug}/users/${session.data.user.id}#cabinet`" class="px-3 py-2 text-sm flex items-center gap-2 hover:bg-black/5 dark:hover:bg-white/10" @click="userMenu.hide()">
