...
import { buildProfileCardElement, type ProfileCardInput } from '../server/utils/share/profile-template'
import type { VNode } from '../server/utils/share/template'

function card(over: Partial<ProfileCardInput> = {}): ProfileCardInput {
  return {
    locale: 'en',
    displayName: 'alice',
...
function textOf(data: ProfileCardInput): string {
  const el = buildProfileCardElement(data, { host: 'goal.example.com', markDataUri: null }, shareTranslator(data.locale))
  const out: string[] = []
  collectText(el, out)
  return out.join(' | ')
}
...
describe('buildProfileCardElement', () => {
  it('renders name, competition, points, rank, exacts, haul and host', () => {
  ...
  it('drops the rank stat for an unranked user', () => {
    const text = textOf(card({ rank: null }))
    expect(text).not.toContain('#')
  })

  it('drops the haul stat when nothing was earned', () => {
    const text = textOf(card({ trophies: 0, badges: 0 }))
    expect(text).not.toContain('haul')
  })
  ...
  it('embeds the brand mark image when a mark data URI is supplied', () => {
    ...
    const el = buildProfileCardElement(card(), { host: 'goal.example.com', markDataUri: mark }, shareTranslator('en'))
