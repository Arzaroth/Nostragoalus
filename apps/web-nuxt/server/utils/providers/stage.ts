...
// tournament" (UEFA's group stage) and "third place" must be tested before the
// bare "final". Previously fifa.ts and uefa.ts kept separate, differently
// ordered ladders - a latent divergence.
const STAGE_KEYWORDS: { match: (s: string) => boolean; stage: AppStage }[] = [
  { match: (s) => s.includes('final tournament'), stage: 'GROUP' },
  { match: (s) => s.includes('third'), stage: 'THIRD_PLACE' },
