...
describe('hasLiveWindow', () => {
  ...
  it('is true for an in-play match', async () => {
    ...
    await makeMatch(db, { competitionId, roundId, kickoffTime: new Date('2026-06-11T19:00:00Z'), status: 'LIVE' })
    expect(await hasLiveWindow(db, NOW)).toBe(true)
    await client.close()
  })

  it('is true for a scheduled match kicked off within the last 4 hours', async () => {
    const { db, client, competitionId, roundId } = await setup()
    await makeMatch(db, { competitionId, roundId, kickoffTime: new Date('2026-06-11T19:00:00Z'), status: 'SCHEDULED' })
    expect(await hasLiveWindow(db, NOW)).toBe(true)
    await client.close()
  })
  ...
  it('is false for future or long-past scheduled matches', async () => {
    const { db, client, competitionId, roundId } = await setup()
    await makeMatch(db, { competitionId, roundId, kickoffTime: new Date('2026-06-11T23:00:00Z'), status: 'SCHEDULED' })
    await makeMatch(db, { competitionId, roundId, kickoffTime: new Date('2026-06-11T10:00:00Z'), status: 'SCHEDULED' })
    expect(await hasLiveWindow(db, NOW)).toBe(false)
    await client.close()
  })
})
...
describe('prediction locking', () => {
  it('locks predictions whose kickoff has passed and unlocks future ones', async () => {
    ...
    const past = await makeMatch(db, { competitionId, roundId, kickoffTime: new Date('2026-06-11T16:00:00Z') })
    const future = await makeMatch(db, { competitionId, roundId, kickoffTime: new Date('2026-06-12T16:00:00Z') })
